...
2026-03-13 05:11 | validation | spec 009 implementation checks | Passed `npm run typecheck`, `npm test -- mcp`, and full `npm test` (95/95) after wiring MCP transport, tools, and config gating.
2026-03-13 05:12 | milestone | accepted spec 009 and closed MCP server scope | Archived spec 009, reset roadmap to no active spec, refreshed KB handoff, and confirmed MCP access now rides on the stable backend without changing existing HTTP or WS contracts.
