...
## Current Focus
- Execute Spec `003`, which adds chain reads, unread aggregation, and mark-read handling on top of the accepted write path and `u-db` adapter boundary.
...
- Letting Spec `003` add complexity that hides the intentionally brute-force MVP unread model.
...
- Expanding scope before the read/unread contract is stable.
