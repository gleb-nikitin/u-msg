...
## Current Focus
- Execute Spec `005` for temporary search/session surfaces and operational hardening while preserving current protocol and provider-neutral boundaries.
...
- Letting Spec `005` overreach into a permanent session model or provider behavior before MVP closure.
...
- Expanding scope before temporary search/session contracts and ops checks are locked.
